//
//  UIColor+Additions.swift
//  Mobal iOS
//
//  Generated on Zeplin. (2021. 4. 10.).
//  Copyright (c) 2021 __MyCompanyName__. All rights reserved.
//

import UIKit

extension UIColor {

  @nonobjc class var backgroundColor: UIColor {
    return UIColor(red: 21.0 / 255.0, green: 21.0 / 255.0, blue: 22.0 / 255.0, alpha: 1.0)
  }
    
    @nonobjc class var brightYellow50: UIColor {
        return UIColor(red: 255/255, green: 245/255, blue: 0, alpha: 0.5)
    }

  @nonobjc class var purpleishBlue: UIColor {
    return UIColor(red: 109.0 / 255.0, green: 57.0 / 255.0, blue: 1.0, alpha: 1.0)
  }

  @nonobjc class var wheat: UIColor {
    return UIColor(red: 1.0, green: 228.0 / 255.0, blue: 135.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var yellowTan80: UIColor {
    return UIColor(red: 1.0, green: 224.0 / 255.0, blue: 116.0 / 255.0, alpha: 0.8)
  }

  @nonobjc class var brownishGrey: UIColor {
    return UIColor(white: 97.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var darkGrey: UIColor {
    return UIColor(red: 34.0 / 255.0, green: 34.0 / 255.0, blue: 36.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var white: UIColor {
    return UIColor(white: 253.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var black: UIColor {
    return UIColor(white: 43.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var brownGrey: UIColor {
    return UIColor(white: 157.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var black40: UIColor {
    return UIColor(white: 16.0 / 255.0, alpha: 0.4)
  }

  @nonobjc class var blackTwo: UIColor {
    return UIColor(white: 34.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var lightBluishGreen: UIColor {
    return UIColor(red: 122.0 / 255.0, green: 1.0, blue: 151.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var black94: UIColor {
    return UIColor(white: 29.0 / 255.0, alpha: 0.94)
  }

  @nonobjc class var brownGreyTwo: UIColor {
    return UIColor(white: 121.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var veryLightPink: UIColor {
    return UIColor(white: 216.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var white7: UIColor {
    return UIColor(white: 1.0, alpha: 0.07)
  }

  @nonobjc class var white20: UIColor {
    return UIColor(white: 1.0, alpha: 0.2)
  }

  @nonobjc class var black25: UIColor {
    return UIColor(white: 0.0, alpha: 0.25)
  }

  @nonobjc class var darkGreyTwo: UIColor {
    return UIColor(red: 41.0 / 255.0, green: 41.0 / 255.0, blue: 44.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var greyishBrown: UIColor {
    return UIColor(white: 71.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var blackThree: UIColor {
    return UIColor(white: 37.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var lemonLime80: UIColor {
    return UIColor(red: 187.0 / 255.0, green: 1.0, blue: 41.0 / 255.0, alpha: 0.8)
  }

  @nonobjc class var lightBluishGreenTwo: UIColor {
    return UIColor(red: 123.0 / 255.0, green: 1.0, blue: 151.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var whiteTwo: UIColor {
    return UIColor(white: 1.0, alpha: 1.0)
  }

  @nonobjc class var blackFour: UIColor {
    return UIColor(white: 0.0, alpha: 1.0)
  }

  @nonobjc class var coral: UIColor {
    return UIColor(red: 1.0, green: 72.0 / 255.0, blue: 72.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var veryLightPinkTwo: UIColor {
    return UIColor(white: 196.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var barbiePink: UIColor {
    return UIColor(red: 1.0, green: 56.0 / 255.0, blue: 176.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var custard: UIColor {
    return UIColor(red: 1.0, green: 250.0 / 255.0, blue: 120.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var darkGreyThree: UIColor {
    return UIColor(red: 32.0 / 255.0, green: 32.0 / 255.0, blue: 33.0 / 255.0, alpha: 1.0)
  }

  @nonobjc class var pastelGreen50: UIColor {
    return UIColor(red: 167.0 / 255.0, green: 1.0, blue: 159.0 / 255.0, alpha: 0.5)
  }

  @nonobjc class var darkCream: UIColor {
    return UIColor(red: 1.0, green: 230.0 / 255.0, blue: 144.0 / 255.0, alpha: 1.0)
  }
        
    @nonobjc class var dandelion: UIColor {
      return UIColor(red: 248.0 / 255.0, green: 233.0 / 255.0, blue: 9.0 / 255.0, alpha: 1.0)
    }
}
